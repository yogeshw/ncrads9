...
"""HLSFrame class for HLS compositing."""
...
@dataclass
class HLSChannelSettings:
    """Settings for an individual HLS channel."""
...
class HLSFrame:
    """Frame for HLS (Hue, Lightness, Saturation) compositing."""

    def __init__(self, frame_id: int, name: str = "") -> None:
        """Initialize an HLSFrame.
        ...
        self._name = name or f"HLS Frame {frame_id}"
        self._hue_frame: Optional[Frame] = None
        self._lightness_frame: Optional[Frame] = None
        self._saturation_frame: Optional[Frame] = None
        self._hue_settings = HLSChannelSettings()
        self._lightness_settings = HLSChannelSettings()
        self._saturation_settings = HLSChannelSettings()
    ...
    @property
    def lightness_frame(self) -> Optional[Frame]:
        """Return the lightness channel frame."""
        return self._lightness_frame

    @lightness_frame.setter
    def lightness_frame(self, frame: Optional[Frame]) -> None:
        """Set the lightness channel frame."""
        self._lightness_frame = frame
        self._composite = None
    ...
    @property
    def hue_settings(self) -> HLSChannelSettings:
        """Return the hue channel settings."""
        return self._hue_settings

    @property
    def lightness_settings(self) -> HLSChannelSettings:
        """Return the lightness channel settings."""
        return self._lightness_settings

    @property
    def saturation_settings(self) -> HLSChannelSettings:
    ...
    def set_channel(self, channel: str, frame: Optional[Frame]) -> None:
        ...
        Args:
            channel: Channel name ('hue', 'lightness', or 'saturation').
            frame: The frame to use for this channel.
        """
        channel_lower = channel.lower()
        if channel_lower in ("hue", "h"):
            self.hue_frame = frame
        elif channel_lower in ("lightness", "l"):
            self.lightness_frame = frame
    ...
    def compose(self) -> Optional[NDArray[np.uint8]]:
        """Compose the RGB image from the HLS channel frames.
        ...
        for frame in [self._hue_frame, self._lightness_frame, self._saturation_frame]:
        ...
        # Default values for HLS
        h_data = np.zeros(shape, dtype=np.float64)
        l_data = np.full(shape, 0.5, dtype=np.float64)
        ...
        if (
            self._lightness_frame is not None
            and self._lightness_frame.image_data is not None
            and self._lightness_settings.visible
        ):
            data = self._lightness_frame.image_data.astype(np.float64)
            if data.shape[:2] == shape:
                l_data = self._normalize_channel(data, self._lightness_settings)
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                r, g, b = colorsys.hls_to_rgb(h_data[i, j], l_data[i, j], s_data[i, j])
                rgb[i, j] = [r, g, b]
