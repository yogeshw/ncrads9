...
TERMINAL_HELP_FLAGS = frozenset({"-h", "--help", "-help"})

#: Flags that open the fuller HTML reference in a browser.
HTML_HELP_FLAGS = frozenset({"--help-html", "-help-html"})


def _cli_help_requested(argv: Sequence[str]) -> bool:
    """Return True when either form of CLI help was requested."""
    flags = TERMINAL_HELP_FLAGS | HTML_HELP_FLAGS
    return any(str(arg).lower() in flags for arg in list(argv)[1:])


def _html_help_requested(argv: Sequence[str]) -> bool:
    """Return True when the browser help page was requested."""
    return any(str(arg).lower() in HTML_HELP_FLAGS for arg in list(argv)[1:])


def format_cli_help(prog: str = "ncrads9") -> str:
    """Return the terminal usage text.

    DS9 takes an ordered stream of options and filenames rather than a fixed
    grammar, so this is written by hand instead of generated by argparse: the
    order of options is significant and argparse cannot express that.
    """
    return f"""\
usage: {prog} [options] [file ...]

A Python/Qt6 FITS viewer modelled on SAOImageDS9.

Options and filenames are applied in the order given, as in DS9, so
`{prog} -log a.fits -linear b.fits` scales each file differently.

Display:
  -zscale                  zscale limits
  -minmax                  min/max limits
  -scale MODE              linear | log | power | sqrt | squared | asinh |
                           sinh | histequ
  -linear -log -sqrt       shorthands for the above
  -squared -asinh -histequ
  -bin N                   block-average by N (see PLAN.md 3.4)
  -zoom FACTOR             zoom to FACTOR
  -pan X Y                 pan to image coordinates X, Y

Colour:
  -cmap NAME               colormap by name (also -colormap, -color)
  -NAME                    colormap directly, e.g. -heat, -viridis
  -invert / -noinvert      invert the colormap
  -colorbar on|off         colorbar visibility

Frames:
  -frame N                 go to frame N
  -single / -tile / -blink display mode
  -match SCOPE             match frames
  -lock SCOPE              lock frames
  -rgb                     create an RGB frame
  -red / -green / -blue F  load F into an RGB channel

Coordinates:
  -wcs SYSTEM              fk5 | fk4 | icrs | galactic | ecliptic
  -fk5 -fk4 -icrs          shorthands for the above
  -galactic -ecliptic
  -sexagesimal / -degrees  coordinate format

Regions:
  -regions FILE            load a DS9 region file

Other:
  -width N / -height N     window size in pixels
  -save FILE               save the rendered image
  -version                 print the version and exit
  -about                   print the about text and exit
  -h, --help               show this message and exit
  --help-html              open the fuller HTML reference in a browser

Not every DS9 option is implemented yet; see TODO.md for the roadmap.
"""
...
ncrads9 -frame rgb -zoom 2 -pan 1024 1024 image.fits</pre>
  ...
  <h2>Core options</h2>
  <ul>
    <li><code>-h</code>, <code>--help</code>: print a usage summary to the terminal.</li>
    <li><code>--help-html</code>: open this browser help page.</li>
...
def run_application(argv: list[str]) -> int:
    ...
    Returns:
        Exit code.
    """
    if _html_help_requested(argv):
        open_cli_help_in_browser()
        return 0
    if _cli_help_requested(argv):
        sys.stdout.write(format_cli_help(Path(argv[0]).name if argv else "ncrads9"))
        return 0
