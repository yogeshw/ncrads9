...
"""
Bpanda region shape: a panda with box annuli.

DS9's `bpanda x y startangle stopangle nangle innerMajor innerMinor
outerMajor outerMinor nradius [angle]` -- a pie sliced into `nangle` wedges
crossed with `nradius` box annuli. Its plain cousin `panda` uses circles;
this uses boxes, so each ring has two radii and the whole shape may be
rotated.

DS9 requires the inner and outer boxes to have the same axis ratio, and
...
class Bpanda(BaseRegion):
    """A panda whose annuli are boxes."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            inner_major: The inner box's semi-major axis.
            inner_minor: The inner box's semi-minor axis.
            outer_major: The outer box's semi-major axis.
            outer_minor: The outer box's semi-minor axis.
    ...
    def inner_major(self) -> float:
        """The inner box's semi-major axis."""
    ...
    def inner_minor(self) -> float:
        """The inner box's semi-minor axis."""
    ...
    def outer_major(self) -> float:
        """The outer box's semi-major axis."""
    ...
    def outer_minor(self) -> float:
        """The outer box's semi-minor axis."""
    ...
    def draw(self, context: Any) -> None:
        """
        Draw the bpanda on the given context.
    ...
    def contains(self, x: float, y: float) -> bool:
        """
        Whether a point lies in one of the bpanda's wedges.
        ...
        Returns:
            True if the point is between the inner and outer boxes and
        ...
        # Inside the outer box and outside the inner one, in the shape's
        # own rotated frame.
        if abs(local_x) > self._outer_major or abs(local_y) > self._outer_minor:
            return False
        if abs(local_x) < self._inner_major and abs(local_y) < self._inner_minor:
            return False
    ...
    def move(self, dx: float, dy: float) -> None:
        """
        Move the bpanda by the given offset.
    ...
    def resize(self, scale_x: float, scale_y: float) -> None:
        """
        Resize the bpanda by the given scale factors.
    ...
    def to_ds9_string(self) -> str:
        """
        Convert the bpanda to a DS9 format string.

        Returns:
            The bpanda as a DS9 format string.
        """
        cx, cy = self.center
        return (
            f"bpanda({cx:g},{cy:g},{self._start_angle:g},{self._stop_angle:g},"
    ...
    def __repr__(self) -> str:
        """Return a string representation of the bpanda."""
        return (
            f"Bpanda(center={self.center}, angles=({self._start_angle},"
