...
def convert_fk4_to_fk5(
    ...
    fk4_frame = FK4(equinox=Time(equinox, format="byear"), obstime=Time(epoch, format="byear"))
...
def convert_fk5_to_fk4(
    ...
    fk4_coord = coord.transform_to(
        FK4(equinox=Time(1950.0, format="byear"), obstime=Time(epoch, format="byear"))
    )
...
def precess_fk4(ra: float, dec: float, from_equinox: float, to_equinox: float) -> Tuple[float, float]:
    ...
    Returns:
        Tuple of (ra, dec) in degrees at the target equinox.
    """
    from_frame = FK4(equinox=Time(from_equinox, format="byear"), obstime=Time(from_equinox, format="byear"))
    to_frame = FK4(equinox=Time(to_equinox, format="byear"), obstime=Time(to_equinox, format="byear"))
