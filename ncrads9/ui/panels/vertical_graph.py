...
class VerticalGraph(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        #: The row the cursor is in, kept so a changed setting can re-cut
    ...
    def update_cursor_position(self, x: float, y: float) -> None:
        ...
        values = cut(self._current_image, "vertical", ix, self.settings)
        if values is None:
            self._info_label.setText("X: ---")
            return
        self._current_x = ix
        self._graph_widget.set_data(values, iy)
        self._info_label.setText(self._describe(ix))

    def _describe(self, index: int) -> str:
        """The label above the graph: where the cut is, and how thick."""
        if self.settings.thickness > 1:
            return f"X: {index} ({self.settings.thickness} {self.settings.method})"
        return f"X: {index}"

    def _apply_size(self) -> None:
        """Fix the across-the-cut dimension to DS9's `graph size`."""
        self.setFixedWidth(self.settings.size)
